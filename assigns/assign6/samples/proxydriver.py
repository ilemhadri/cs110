...
class RequestResult(object):
    ...
    def print_result(self, hash_body=False):
        ...
        if hash_body:
            ...
            print("Body:")
            self.body = self.body.replace('13 171', '13, 171')
            self.body = self.body.replace('13,171', '13, 171')
            self.body = self.body.replace('13:171', '13, 171')
            print(self.body)
            print('')
...
class ProxyTest(object):
    def __init__(self, port):
        ...
        self.blocked_host = 'http://forbidden.cs110-proxy.jerrycainjr.com'

        self.proxy = '%s:%d' % (socket.gethostname(), port)
...
def main():
    ...
    #that = "myth64.stanford.edu"
    #if that == socket.gethostname():
    #    that = "myth65.stanford.edu"
    try:
        if not using_proxy_chaining:
            argv = [proxy_executable, "--port", str(proxy_port)]
            proxy = subprocess.Popen(argv, cwd=proxy_cwd, stdout=log_file, stderr=subprocess.STDOUT)
        else:
            argv = [proxy_executable, "--port", str(proxy_port), "--proxy-server", this, "--proxy-port", str(secondary_proxy_port)]
            proxy = subprocess.Popen(argv, cwd=proxy_cwd, stdout=log_file, stderr=subprocess.STDOUT)
    except:
    ...
    if using_proxy_chaining:
        time.sleep(2) # give time for cache to be created by first proxy
        try:
            #argv1 = ["cd", os.getcwd()]
            argv = [proxy_executable, "--port", str(secondary_proxy_port), "--proxy-server", this, "--proxy-port", str(proxy_port)]
            #command = '"' + " ".join(argv1) + " && " + " ".join(argv2) + '"'
            #argv = ["/bin/bash", "-c", " ".join(["ssh", "-t", that, command])]
            secondary_proxy = subprocess.Popen(argv, cwd=proxy_cwd, stdout=log_file, stderr=subprocess.STDOUT)
        except:
            print("Couldn't start secondary proxy")
            return
        
    # try:
    time.sleep(5 if not using_proxy_chaining else 5) # Make sure the proxy is running
