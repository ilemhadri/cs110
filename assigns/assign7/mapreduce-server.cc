 ...
 */
void MapReduceServer::run() noexcept {
  spawnMappers();
}
...
void MapReduceServer::parseArgumentList(int argc, char *argv[]) noexcept(false) {
...
unsigned short MapReduceServer::computeDefaultPortForUser() const noexcept {
  size_t hashValue = hash<string>()(user + "@" + host); // ensure different than proxy default port
 ...
 */
void MapReduceServer::confirmRequiredArgumentsArePresent(const string& configFilename) const noexcept(false) {
  ostringstream oss;
  if (mapper.empty()) {
 ...
 */
void MapReduceServer::confirmExecutablesAreExecutable() const noexcept(false) {
  struct executable {
...
void MapReduceServer::initializeFromConfigFile(const string& configFileName) noexcept(false) {
  ...
  while (true) {
    string key;
    getline(infile, key, /* stop at char: */ ' ');
...
void MapReduceServer::applyToServer(const string& key, const string& value) noexcept(false) {
  if (key == "mapper") {
 ...
 * influence execution.
 */
void MapReduceServer::logServerConfiguration(ostream& os) noexcept {
  if (!verbose) return;
 ...
 * -------------------------
 * Compiles a map of IP addresses (e.g. "171.64.15.22") to hostname (e.g. "myth61.stanford.edu").
 ...
 */
void MapReduceServer::buildIPAddressMap() noexcept {
 ...
 * Method: listFiles
 * -----------------
 * Examines the named directory for its legit entries and returns a
 * list of the absolute path names of all these files.
 */
vector<string> MapReduceServer::listFiles(const std::string& directory) const noexcept {
 ...
 */
void MapReduceServer::startServer() noexcept(false) {
 ...
 * spawned workers.
 */
void MapReduceServer::orchestrateWorkers() noexcept {
  ...
  while (true) {
    ...
    if (!serverIsRunning) { close(serverSocket); break; }
 ...
 * received.
 */
void MapReduceServer::handleRequest(int clientSocket, const string& clientIPAddress) noexcept {
  if (verbose) 
  ...
  if (message == kWorkerReady) {
    string pattern;
    bool success = getNextFilePattern(pattern);
    if (success) {
      if (verbose)
 ...
 */
bool MapReduceServer::getNextFilePattern(std::string& pattern) noexcept {
 ...
 */
void MapReduceServer::spawnMappers() noexcept {
  for (const std::string &f : listFiles(inputPath)) unprocessed.push(f);
  vector<string> mapperNodes(nodes);
  random_device rng;
  shuffle(mapperNodes.begin(), mapperNodes.end(), mt19937(rng()));
...
string MapReduceServer::buildMapperCommand(const string& remoteHost, 
                                           ...
                                           const string& outputPath) noexcept {
  ostringstream oss;
 ...
 * 
 *  'ssh poohbear@myth55.stanford.edu '/usr/class/cs110/<other-directories>/<worker> <arg1> ... <argn>',
 * 
 * which is precisely the command that can be used to launch a worker--either a mapper or a reducer--
 * on a remote machine (in this example, myth55).
 * 
 ...
 */
void MapReduceServer::spawnWorker(const string& node, const string& command) const noexcept {
  if (verbose)
 ...
 */
void MapReduceServer::dumpFileHashes(const std::string& dir) noexcept {
...
void MapReduceServer::dumpFileHash(const string& file) noexcept {
 ...
 */
MapReduceServer::~MapReduceServer() noexcept {
 ...
 * resources and exits.
 */
void MapReduceServer::bringDownServer() noexcept {
  // assumption is that thread pool is empty and that server is asleep in an accept call
  serverIsRunning = false;
