 ...
 * getNumProcesses function, which just used popen (similar to your
 * subprocess function from the midterm) to ssh to the specified
...
int getNumProcesses(int num, const unordered_set<string>& sunetIDs) {
  ...
  FILE *instream = popen(oss.str().c_str(), "r"); // popen is like your subprocess routine from lecture
  int numProcesses = 0;
