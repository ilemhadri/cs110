...
static void ticketAgent(size_t id, unsigned int& remainingTickets, mutex& ticketsLock) {
  while (true) {
    ticketsLock.lock();
...
int main(int argc, const char *argv[]) {
  ...
  for (size_t i = 0; i < kNumAgents; i++)
    agents[i] = thread(ticketAgent, kBaseIDNumber + i, ref(remainingTickets), ref(ticketsLock));
